...
impl IGetUtils for GetUtils {
    ...
    fn get_user_id_and_language(
        ...
        let user_id = self.get_user_id(cookies)?;
        let language = self.get_user_language(cookies);
        Ok((user_id, language))
    }
    ...
    async fn get_performance_value_and_graph_data(
        ...
        if input_first_date.is_none() || input_last_date.is_none() {
            (first_date, last_date) =
                Self::get_first_date_and_last_date_from_bank(&self, Some(&all_transactions));
        } else {
    ...
    async fn get_contracts_with_history(
        ...
        for contract in contracts.iter() {
            ...
            let total_amount_paid =
                Self::get_total_amount_paid_of_contract(&self, contract.id, language, db).await?;
